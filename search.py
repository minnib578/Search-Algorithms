...
"""
 ...
 Unique operations of OrderedSet:  (LIFO)
 ...
 Unique operations of Stack:(LIFO)
 ...
 Unique operations of Queue: (FIFO)
...
def path_backtrack(start,child,parents,actions):                                                # find parent and action
    movement=[]
    while (child !=start):

        movement.append(actions[child[0]][child[1]])
...
def depth_first_search(grid_size, start, goal, obstacles, costFn, logger):
    ...
    while len(open_set)>0:
        ...
        children= [(x-1, y), (x, y-1), (x+1, y), (x, y+1)]                                    # possible children(up,left,down,right)

        boundary = [(x,y) for x in range(n_rows) for y in range(n_cols)]                      # boundary restriction
        ...
        for child in children:
            ...
            if  child in boundary and child not in obstacles :
                ...
                if child not in open_set and child not in closed_set :
                    ...
                    parents[child[0]][child[1]]=node                                          # remark parent for current children

                    actions[child[0]][child[1]]=(child[0]-node[0],child[1]-node[1])           # remark children action

                    if child == goal:

                        movement=path_backtrack(start,child,parents,actions)                  # backtrack to find the path
        ...
        for child in children:
            ...
            if child in boundary and child not in obstacles :
                ...
                if child not in open_set and child not in closed_set :
                    ...
                    actions[child[0]][child[1]]=(child[0]-node[0],child[1]-node[1])           # actions remark

                elif child in open_set and child not in closed_set:                           # update that OPEN_node

                    if g_child<open_set.get(child):                                           # update the cost and parent,action of child existed in open_set

                        open_set.put(child,g_child)
                        parents[child[0]][child[1]]=node
                        actions[child[0]][child[1]]=(child[0]-node[0],child[1]-node[1])
...
def astar_search(grid_size, start, goal, obstacles, costFn, logger):
    ...
    open_set = PriorityQueue(order="min", f=lambda v: v)                                         # priority data structure
    closed_set = OrderedSet()                                                                    # OrderedSet data Structure
    ...
    while len(open_set)>0:
        ...
        for child in children:
            ...
            g_parent=f-h_parent
            f_child=heuristic(child[0],child[1])+g_parent+costFn((child[0],child[1]))
            ...
            if  child in boundary and child not in obstacles :
                ...
                if child not in open_set and child not in closed_set :
                    ...
                    parents[child[0]][child[1]]=node                                               #  remark parent for child

                    actions[child[0]][child[1]]=(child[0]-node[0],child[1]-node[1])                # remark actions for child

                elif child in open_set and child not in closed_set:

                        if f_child<open_set.get(child):                                            # update parent, action,cost of child existed in open_set
                            open_set.put(child,f_child)
                            parents[child[0]][child[1]]=node
                            actions[child[0]][child[1]]=(child[0]-node[0],child[1]-node[1])
...
if __name__ == "__main__":
    ...
    assert(ACTIONS == App.ACTIONS),'The actions is not math'
